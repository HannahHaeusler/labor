...
//        .\gradlew jibDockerBuild [-D'tag=2.0.0'] [-Ddebug=true]
//        .\gradlew bootBuildImage
//              erfordert die lokale Windows-Gruppe docker-users
//              docker run --publish 8080:8080 --env TZ=Europe/Berlin --name dok --rm dok:1.0-GraalVM
...
tasks.bootBuildImage {
    ...
    val dockerAccount = "juergenzimmermann"
    val tag = System.getProperty("tag") ?: project.version
    imageName = "docker.io/${dockerAccount}/${project.name}:$tag"
...
jib {
    ...
    to {
        val dockerAccount = "juergenzimmermann"
        val tag = System.getProperty("tag") ?: project.version
        image = "docker.io/${dockerAccount}/${project.name}:$tag"
        if (debug.toBoolean()) {
            image += "-debug"
        }
    }
...
tasks.jibDockerBuild.get().setDependsOn(listOf(tasks.bootJar))
